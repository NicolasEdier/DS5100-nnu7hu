...
# Student Info
    ...
    Name: Nicolas Edier
    Net ID: nnu7hu
    File GitHub URL: https://github.com/NicolasEdier/DS5100-nnu7hu/blob/main/lessons/M11/M11-HW-2.Rmd
...
abalone_data <- read.csv(url, header = FALSE)
head(abalone_data)
```
...
```{r}
max(abalone_data[, ncol(abalone_data)])
```
...
```{r}
nrow(filter(abalone_data, V1 == "I" | V1 == "M"))
```
...
```{r}
V2_mean_by_gender <- abalone_data %>%
  group_by(V1) %>%
  summarize(mean_V2 = mean(V2, na.rm = TRUE))

print(V2_mean_by_gender)
```
...
```{r}
V2_median_males <- abalone_data %>%
  filter(V1 == "M") %>%
  summarize(median_males_V2 = median(V2))

print(V2_median_males)
```
